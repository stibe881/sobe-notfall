/**
 * Gemeinsame Domänentypen für Server, Webportal und App.
 *
 * Bewusst NICHT hier: `User`/`StoredUser` (Server hat interne Passwortfelder,
 * jeder Client hat seine eigene, flache Sicht auf den Benutzer),
 * `IntegrationSettings` für die App (andere Optionalität, weil die App auch
 * ältere Alarmserver unterstützt) sowie `AppState`/`ServerState` (je nach
 * Client bzw. Server unterschiedlich zusammengesetzt). Diese Typen bleiben
 * bewusst lokal in server/src/types.ts, src/types.ts und mobile/src/types.ts.
 */
...
/**
 * Vollständige Integrationseinstellungen, wie Server und Portal sie
 * austauschen. Die App hat eine eigene, defensivere Fassung (ältere
 * Alarmserver liefern nicht jedes Feld) – siehe mobile/src/types.ts.
 */
