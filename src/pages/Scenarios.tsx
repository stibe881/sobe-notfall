...
import { ClipboardList, Eye, EyeOff, Pencil, Phone, Plus, Scale, Trash2, Users } from 'lucide-react'
...
function SzenarioWizard({ onClose }: { onClose: () => void }) {
  ...
  // Jedes Szenario braucht einen Alarmplan – entweder neu (Kanäle/Gruppen vom Szenario übernommen)
  // oder ein bereits bestehender, noch freier Plan (falls vorhanden)
  const [planModus, setPlanModus] = useState<'neu' | 'bestehend'>('neu')
  const [planName, setPlanName] = useState('')
  const [gewaehlterPlanId, setGewaehlterPlanId] = useState('')
  const freiePlaene = state.plans.filter((p) => !p.scenarioId)
  ...
  function erstellen() {
    ...
    if (planModus === 'neu') {
      ...
      const plan = state.plans.find((p) => p.id === gewaehlterPlanId)
      if (plan) dispatch({ type: 'UPSERT_PLAN', plan: { ...plan, scenarioId: draft.id } })
    }
    onClose()
  }
  ...
  const gewaehlterPlan = state.plans.find((p) => p.id === gewaehlterPlanId)
  ...
  return (
    <Wizard
      ...
      schritte={[
        ...
        {
          titel: 'Alarmplan',
          hinweis: 'Jedes Szenario braucht einen Alarmplan – er wird beim Auslösen automatisch vorgeschlagen und regelt die Eskalation.',
          gueltig: planModus === 'neu' ? !!planName.trim() : !!gewaehlterPlanId,
          inhalt: (
            <>
              {freiePlaene.length > 0 && (
                <div className="flex gap-2 mb-3">
                  <AuswahlChip aktiv={planModus === 'neu'} onClick={() => setPlanModus('neu')}>Neuen Plan erstellen</AuswahlChip>
                  <AuswahlChip aktiv={planModus === 'bestehend'} onClick={() => setPlanModus('bestehend')}>Bestehenden freien Plan verwenden</AuswahlChip>
                </div>
              )}
              {planModus === 'neu' ? (
                <Field label="Name des Alarmplans">
                  <input
                    ...
                    placeholder={`z. B. Alarmplan ${draft.title || 'Szenario'}`}
                    value={planName}
                ...
                <Field label="Bestehender, noch freier Alarmplan">
                  <select className={inputClass} value={gewaehlterPlanId} onChange={(e) => setGewaehlterPlanId(e.target.value)}>
                    <option value="">– bitte wählen –</option>
                    {freiePlaene.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}
                  </select>
                </Field>
              )}
            </>
            ...
            <>
              <div className="rounded-xl border border-brand-200 bg-brand-50/60 p-4">
                ...
                <div className="text-xs text-slate-600 mt-1">
                  ...
                  Alarmplan: {planModus === 'neu' ? (planName.trim() || 'Ohne Namen') : (gewaehlterPlan?.name ?? '–')}
                </div>
              </div>
